...
class GlobalAdsPage extends Page implements HasForms, HasTable
{
    use InteractsWithForms, InteractsWithTable;

    protected static ?string $navigationIcon = 'heroicon-o-sparkles';
    protected static ?string $navigationLabel = 'الإعلانات العامة';
    protected static ?string $navigationGroup = 'إعدادات الإعلانات';
    protected static ?int $navigationSort = 105;
    protected static string $view = 'filament.pages.ads-page';
    protected static ?string $title = 'الإعلانات العامة والإضافية';
    protected static ?string $slug = 'global-ads';

    public function table(Table $table): Table
    {
        return $table
            ->query(AdSlot::query()->whereIn('page_type', ['global', 'result'])->orderBy('page_type')->orderBy('sort_order'))
            ->columns([
                ...
                Tables\Columns\TextColumn::make('page_type')
                    ...
                    ->color(fn (string $state): string => $state === 'global' ? 'success' : 'danger'),
            ...
            ->heading('✨ الإعلانات العامة وصفحات النتائج')
            ->description('إعلانات تظهر في جميع الصفحات (مثل الإعلان المعلق) وإعلانات صفحات النتائج الفردية');
    }
}
