...
<div class="container mx-auto px-4 py-8">
    ...
    @if($serviceType === 'embed' && isset($examType))
        {{-- Embed/iFrame Mode --}}
        @include('partials.result-embed', ['examType' => $examType, 'title' => $title ?? 'نتيجة الشهادة الإعدادية'])
    @elseif($serviceType === 'pdf' && isset($examType))
        {{-- PDF Viewer Mode --}}
        @include('partials.result-pdf', ['examType' => $examType, 'title' => $title ?? 'نتيجة الشهادة الإعدادية'])
    @elseif($serviceType === 'governorate_table' && isset($examType))
        {{-- Governorate Table Mode --}}
        @include('partials.result-governorate-table', ['examType' => $examType, 'title' => $title ?? 'نتيجة الشهادة الإعدادية'])
    @else
        {{-- Default Search Mode --}}
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8">
                <h1 class="text-2xl md:text-3xl font-black text-center text-gray-800 mb-4 leading-relaxed">
                    @if(isset($governorate))
                        نتيجة الشهادة الإعدادية في منطقة {{ $governorate->name_ar }} في ليبيا 2026 الفصل الدراسي الأول
                    @else
                        نتيجة الشهادة الإعدادية في ليبيا 2026 الفصل الدراسي الأول
                    @endif
                </h1>
                
                <!-- Result Timer -->
                <x-result-timer country="libya" type="preparatory" />
                ...
                <form @submit.prevent="search" class="space-y-4">
                    <div class="flex flex-col sm:flex-row gap-4">
                        ...
                        <div class="w-full sm:w-1/3">
                            ...
                            <select x-model="academicYearId" 
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 focus:outline-none text-lg transition-all bg-white">
                    ...
                    <div class="w-full {{ (isset($showYearFilter) && $showYearFilter) ? 'sm:w-2/3' : '' }}">
                        ...
                        <input type="text" 
                               ...
                               class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-emerald-500 focus:outline-none text-lg">
                    </div>
                </div>

                <button type="submit" 
                        :disabled="loading"
                        class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-6 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <div class="bg-gray-50 rounded-lg p-6 border-2 border-gray-200">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                ...
                                <p class="text-gray-600">المنطقة: <span x-text="result.governorate"></span></p>
                            </div>
                            <div class="text-left">
                                <p class="text-3xl font-bold text-emerald-600" x-text="result.total_score"></p>
...
function searchComponent() {
    return {
        ...
        async search() {
            ...
            try {
                // Here we might need a dedicated endpoint or logic to get the exam type ID properly.
                // ideally passed from controller, but for now we fetch by code convention logic if possible
                // OR we can just hardcode the logic to find 'ly_preparatory' on backend if search is generic.
                // But simplified:
                const response = await fetch('{{ route("search") }}', {
                    ...
                    body: JSON.stringify({
                        query: this.query,
                        exam_type_code: 'ly_preparatory', // Helper to find ID on backend or we fetch ID first
                ...
                if (data.success) {
                    ...
                    if (this.results.length > 0) {
                        ...
                        const newUrl = '/libya/preparatory/student/' + firstSeat;
...
@endpush
    ...
    <div class="w-full max-w-6xl mx-auto mt-8 px-3">
        <div class="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl shadow-lg p-6 md:p-10 border border-emerald-100">
            @if($governorate->content_title)
            <h2 class="text-xl md:text-2xl lg:text-3xl font-black text-emerald-800 mb-5 pb-3 border-b-2 border-emerald-200 flex items-center gap-3">
                <i class="fa-solid fa-map-location-dot text-emerald-600"></i>
                {{ $governorate->content_title }}
            </h2>
            @endif
            @if($governorate->content_intro)
            <p class="text-emerald-700 mb-6 text-base md:text-lg leading-relaxed">{{ $governorate->content_intro }}</p>
            @endif
            @if($governorate->content_body)
            <div class="prose prose-base md:prose-lg max-w-none text-gray-700 leading-loose
                        prose-headings:font-bold prose-headings:text-emerald-800 prose-headings:mt-6 prose-headings:mb-3
                        prose-h2:text-xl prose-h2:md:text-2xl prose-h2:border-r-4 prose-h2:border-emerald-500 prose-h2:pr-4 prose-h2:py-1
                        prose-h3:text-lg prose-h3:md:text-xl prose-h3:text-emerald-700
                        prose-p:mb-4 prose-p:text-base prose-p:md:text-lg
                        prose-ul:my-4 prose-ul:pr-6 prose-li:mb-2 prose-li:text-base prose-li:md:text-lg
