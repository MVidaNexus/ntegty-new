{{--
    Internal Linking Section for Egyptian Governorates
    This partial creates SEO-friendly internal links to all governorate result pages
    Accepts: $currentGovernorateSlug (optional) - to exclude current governorate from the list
--}}
@php
...
<div class="w-full max-w-6xl mx-auto mt-12 px-3 no-print">
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
        ...
        {{-- Governorates Table/List --}}
        <div class="divide-y divide-gray-100">
            @foreach($egyptGovernorates as $index => $gov)
                @if($gov['slug'] !== $currentSlug)
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="font-bold text-gray-800 group-hover:text-emerald-700 transition-colors">
                                نتيجة الشهادة الإعدادية محافظة {{ $gov['name'] }}
                            </h3>
                            <p class="text-xs text-gray-500 mt-0.5">
                                <i class="fa-solid fa-graduation-cap ml-1 text-emerald-500"></i>
                                الترم الأول والثاني {{ date('Y') }}
                            </p>
                        </div>
                    </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2 text-xs text-gray-500">
                    <span class="bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full font-bold">
                        ...
                        {{ count($egyptGovernorates) - ($currentSlug ? 1 : 0) }} محافظة
                    </span>
                </div>
            </div>
        </div>
