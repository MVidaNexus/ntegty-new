...
<div class="container mx-auto px-4 py-8">
    ...
    @if($serviceType === 'embed' && isset($examType))
        {{-- Embed/iFrame Mode --}}
        @include('partials.result-embed', ['examType' => $examType, 'title' => $title ?? $examType->name_ar])
    @elseif($serviceType === 'pdf' && isset($examType))
        {{-- PDF Viewer Mode --}}
        @include('partials.result-pdf', ['examType' => $examType, 'title' => $title ?? $examType->name_ar])
    @elseif($serviceType === 'governorate_table' && isset($examType))
        {{-- Governorate Table Mode --}}
        @include('partials.result-governorate-table', ['examType' => $examType, 'title' => $title ?? $examType->name_ar])
    @else
        {{-- Default Search Mode --}}
        @php
        ...
        <div class="bg-gradient-to-br from-white to-blue-50 rounded-2xl sm:rounded-3xl shadow-2xl p-5 sm:p-8 md:p-10 border border-blue-100">
            ...
            <div class="text-center mb-6 sm:mb-8 no-print">
                ...
                <div class="mb-4 sm:mb-6">
                    <x-result-timer :country="$country->slug" :type="$examType->level ?? 'secondary'" :governorate="$governorate->slug ?? null" />
                </div>
            ...
            {{-- Ad: After Title --}}
            <x-ad-unit slug="country-after-title" />

            <!-- Search Form -->
            {{-- Ad: Before Search --}}
            <x-ad-unit slug="country-before-search" />
            ...
            <form @submit.prevent="search" class="space-y-4 sm:space-y-6 no-print">
                ...
                {{-- Ad: Inside Search --}}
                <x-ad-unit slug="country-inside-search" />
            ...
            {{-- Ad: After Search --}}
            <x-ad-unit slug="country-after-search" />
                        ...
                        <div class="hidden print:block mb-6 border-b-2 border-gray-800 pb-4">
                            <div class="flex items-center justify-between px-4">
                                <div class="flex items-center gap-3">
                                    <img src="{{ asset('uploads/' . $country->flag_path) }}" class="h-12 w-auto object-contain">
                                    <div class="text-right">
                                        <h2 class="text-lg font-bold text-gray-900 leading-tight">نتيجتي - {{ $country->name_ar }}</h2>
                                        <p class="text-xs text-gray-600">{{ $examType->name_ar }}</p>
                                    </div>
                                </div>
                                <div class="text-left">
                                    @if(isset($settings['logo']))
                                        <img src="{{ asset('uploads/' . $settings['logo']) }}" class="h-14 w-auto object-contain">
                                    @else
                        ...
                        <div class="mb-4 sm:mb-5">
                            ...
                            <div class="pt-6 border-t border-gray-200 print:border-gray-800">
                                ...
                                <div class="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 no-print">
                                    ...
                                    <!-- Certificate Button -->
                                    <a :href="`{{ route('certificate.index') }}?name=${encodeURIComponent(result.student_name)}&score=${result.total_score}&school=${encodeURIComponent(result.governorate || '')}&type={{ urlencode($examType->name_ar) }}`" 
                                       target="_blank"
                                ...
                                <div class="mt-4 text-center no-print">
                                    <button @click="const link = '{{ route('country.exam', ['country' => $country->slug, 'slug' => $examType->slug]) }}/' + result.seat_number; navigator.clipboard.writeText(link); $el.innerHTML = '<i class=\'fa-solid fa-check text-green-500\'></i> تم نسخ الرابط بنجاح'; setTimeout(() => $el.innerHTML = '<i class=\'fa-solid fa-link\'></i> نسخ الرابط المباشر للنتيجة', 3000)" 
                            ...
                            <div class="hidden print:block text-center mt-6 pt-4 border-t-2 border-gray-800">
                                ...
                                <p class="text-xs text-gray-500">تم استخراج النتيجة بتاريخ {{ date('Y-m-d H:i') }}</p>
                            </div>
                        </div>
                    </template>
                </div>
...
function searchComponent() {
    return {
        ...
        academicYearId: localStorage.getItem('academic_year_id') || '{{ \App\Models\AcademicYear::where("is_active", true)->value("id") }}',
        loading: false,
        results: [],
        error: '',
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (this.results.length === 0) {
                        ...
                        const newUrl = '/{{ $country->slug }}/{{ $examType->slug ?? "" }}/student/' + firstSeat;
...
@endpush
    ...
    {{-- ExamType Content Section for SEO --}}
