...
<link href="https://fonts.googleapis.com/css2?family=<?php echo e(str_replace(' ', '+', $fontFamily)); ?>:wght@400;500;600;700;800;900&display=swap" rel="stylesheet">
<?php $__env->stopPush(); ?>
    ...
    <div class="container mx-auto px-4 mb-8">
        <div class="text-center max-w-5xl mx-auto">
            ...
            <h1 class="text-2xl md:text-4xl lg:text-5xl font-black mb-4 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent leading-loose py-4 px-2">
                <?php echo e($s?->page_title ?? 'تصميم شهادة تقدير للمتفوقين'); ?>

            </h1>
            
            <!-- Description -->
            <p class="text-base md:text-xl text-slate-600 font-medium max-w-3xl mx-auto leading-relaxed">
                <?php echo e($s?->page_description ?? 'اصنع ذكرى جميلة لنجاحك واحتفظ بشهادة تقدير بتصميم احترافي في ثوانٍ ✨'); ?>
            ...
            <div class="w-full lg:w-2/3">
                ...
                <div class="mt-6 bg-white rounded-2xl p-6 shadow-xl border-2 border-purple-100/50">
                    ...
                    <div class="flex flex-wrap justify-center gap-3 md:gap-4">
                        <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo e(url()->current()); ?>" target="_blank" class="flex items-center gap-2 bg-[#1877F2] text-white px-5 py-3 rounded-xl hover:opacity-90 hover:-translate-y-1 transition-all font-bold shadow-md hover:shadow-[#1877F2]/30">
                            <i class="fa-brands fa-facebook-f"></i> <span class="hidden md:inline">فيسبوك</span>
                        </a>
                        <a href="https://twitter.com/intent/tweet?url=<?php echo e(url()->current()); ?>&text=حصلت على شهادة تقدير احترافية من موقع نتيجتي! 🎓%0Aجربها الآن مجاناً 👇" target="_blank" class="flex items-center gap-2 bg-black text-white px-5 py-3 rounded-xl hover:bg-gray-800 hover:-translate-y-1 transition-all font-bold shadow-md hover:shadow-black/30">
                            <i class="fa-brands fa-x-twitter"></i> <span class="hidden md:inline">تويتر</span>
                        </a>
                        <a href="https://wa.me/?text=موقع نتيجتي بيعمل شهادات تقدير احترافية مجاناً! 🎓%0Aجربه من هنا: <?php echo e(url()->current()); ?>" target="_blank" class="flex items-center gap-2 bg-[#25D366] text-white px-5 py-3 rounded-xl hover:opacity-90 hover:-translate-y-1 transition-all font-bold shadow-md hover:shadow-[#25D366]/30">
...
document.addEventListener('alpine:init', () => {
    Alpine.data('certificateGenerator', () => ({
        // بيانات المستخدم
        studentName: <?php echo json_encode(request('name', ''), 512) ?>,
        schoolName: <?php echo json_encode(request('school', ''), 512) ?>,
        examType: <?php echo json_encode(request('type', ''), 512) ?>,
        totalScore: <?php echo json_encode(request('score', ''), 512) ?>,
        maxScore: <?php echo json_encode(request('max', request('total', ''))) ?>,
        percentage: <?php echo json_encode(request('percentage', ''), 512) ?>,
        seatNumber: <?php echo json_encode(request('seat', ''), 512) ?>,
        status: <?php echo json_encode(request('status', ''), 512) ?>,
        gender: 'male',
        
        // إعدادات Canvas
        canvas: null,
        ctx: null,
        width: <?php echo e($s?->canvas_width ?? 2480); ?>,
        height: <?php echo e($s?->canvas_height ?? 1754); ?>,
        bgImage: null,
        
        // إعدادات من قاعدة البيانات - البنية الجديدة مع 5 أسطر
        settings: {
            // أسطر الذكور
            line1TextMale: <?php echo json_encode($s?->line1_text_male ?? 'تتقدم إدارة المدرسة والهيئة التعليمية بخالص التهاني والتبريكات', 15, 512) ?>,
            line2TextMale: <?php echo json_encode($s?->line2_text_male ?? 'للطالب المتفوق {student_name}', 15, 512) ?>,
            line3TextMale: <?php echo json_encode($s?->line3_text_male ?? 'وذلك لحصوله على مجموع {total_score} من {max_score}', 15, 512) ?>,
            line4TextMale: <?php echo json_encode($s?->line4_text_male ?? 'في {exam_type}', 15, 512) ?>,
            line5TextMale: <?php echo json_encode($s?->line5_text_male ?? 'متمنين له دوام التوفيق والنجاح', 15, 512) ?>,
            line6TextMale: <?php echo json_encode($s?->line6_text_male ?? '', 15, 512) ?>,
            // أسطر الإناث
            line1TextFemale: <?php echo json_encode($s?->line1_text_female ?? 'تتقدم إدارة المدرسة والهيئة التعليمية بخالص التهاني والتبريكات', 15, 512) ?>,
            line2TextFemale: <?php echo json_encode($s?->line2_text_female ?? 'للطالبة المتفوقة {student_name}', 15, 512) ?>,
            line3TextFemale: <?php echo json_encode($s?->line3_text_female ?? 'وذلك لحصولها على مجموع {total_score} من {max_score}', 15, 512) ?>,
            line4TextFemale: <?php echo json_encode($s?->line4_text_female ?? 'في {exam_type}', 15, 512) ?>,
            line5TextFemale: <?php echo json_encode($s?->line5_text_female ?? 'متمنين لها دوام التوفيق والنجاح', 15, 512) ?>,
            line6TextFemale: <?php echo json_encode($s?->line6_text_female ?? '', 15, 512) ?>,
            
            // تنسيق كل سطر - ذكور
            line1FontFamily: <?php echo json_encode($s?->line1_font_family ?? 'Cairo', 15, 512) ?>,
            line1FontSize: <?php echo e($s?->line1_font_size ?? 50); ?>,
            line1Color: <?php echo json_encode($s?->line1_color ?? '#374151', 15, 512) ?>,
            line1PositionX: <?php echo e($s?->line1_position_x ?? 1240); ?>,
            line1PositionY: <?php echo e($s?->line1_position_y ?? 900); ?>,
            line2FontFamily: <?php echo json_encode($s?->line2_font_family ?? 'Cairo', 15, 512) ?>,
            line2FontSize: <?php echo e($s?->line2_font_size ?? 50); ?>,
            line2Color: <?php echo json_encode($s?->line2_color ?? '#374151', 15, 512) ?>,
            line2PositionX: <?php echo e($s?->line2_position_x ?? 1240); ?>,
            line2PositionY: <?php echo e($s?->line2_position_y ?? 1000); ?>,
            line3FontFamily: <?php echo json_encode($s?->line3_font_family ?? 'Cairo', 15, 512) ?>,
            line3FontSize: <?php echo e($s?->line3_font_size ?? 50); ?>,
            line3Color: <?php echo json_encode($s?->line3_color ?? '#374151', 15, 512) ?>,
            line3PositionX: <?php echo e($s?->line3_position_x ?? 1240); ?>,
            line3PositionY: <?php echo e($s?->line3_position_y ?? 1100); ?>,
            line4FontFamily: <?php echo json_encode($s?->line4_font_family ?? 'Cairo', 15, 512) ?>,
            line4FontSize: <?php echo e($s?->line4_font_size ?? 50); ?>,
            line4Color: <?php echo json_encode($s?->line4_color ?? '#374151', 15, 512) ?>,
            line4PositionX: <?php echo e($s?->line4_position_x ?? 1240); ?>,
            line4PositionY: <?php echo e($s?->line4_position_y ?? 1200); ?>,
            line5FontFamily: <?php echo json_encode($s?->line5_font_family ?? 'Cairo', 15, 512) ?>,
            line5FontSize: <?php echo e($s?->line5_font_size ?? 50); ?>,
            line5Color: <?php echo json_encode($s?->line5_color ?? '#374151', 15, 512) ?>,
            line5PositionX: <?php echo e($s?->line5_position_x ?? 1240); ?>,
            line5PositionY: <?php echo e($s?->line5_position_y ?? 1300); ?>,
            line6FontFamily: <?php echo json_encode($s?->line6_font_family ?? 'Cairo', 15, 512) ?>,
            line6FontSize: <?php echo e($s?->line6_font_size ?? 50); ?>,
            line6Color: <?php echo json_encode($s?->line6_color ?? '#374151', 15, 512) ?>,
            line6PositionX: <?php echo e($s?->line6_position_x ?? 1240); ?>,
            line6PositionY: <?php echo e($s?->line6_position_y ?? 1400); ?>,
            
            // تنسيق كل سطر - إناث
            line1FontFamilyFemale: <?php echo json_encode($s?->line1_font_family_female ?? $s?->line1_font_family ?? 'Cairo', 15, 512) ?>,
            line1FontSizeFemale: <?php echo e($s?->line1_font_size_female ?? $s?->line1_font_size ?? 50); ?>,
            line1ColorFemale: <?php echo json_encode($s?->line1_color_female ?? $s?->line1_color ?? '#374151', 15, 512) ?>,
            line1PositionXFemale: <?php echo e($s?->line1_position_x_female ?? $s?->line1_position_x ?? 1240); ?>,
            line1PositionYFemale: <?php echo e($s?->line1_position_y_female ?? $s?->line1_position_y ?? 900); ?>,
            line2FontFamilyFemale: <?php echo json_encode($s?->line2_font_family_female ?? $s?->line2_font_family ?? 'Cairo', 15, 512) ?>,
            line2FontSizeFemale: <?php echo e($s?->line2_font_size_female ?? $s?->line2_font_size ?? 50); ?>,
            line2ColorFemale: <?php echo json_encode($s?->line2_color_female ?? $s?->line2_color ?? '#374151', 15, 512) ?>,
            line2PositionXFemale: <?php echo e($s?->line2_position_x_female ?? $s?->line2_position_x ?? 1240); ?>,
            line2PositionYFemale: <?php echo e($s?->line2_position_y_female ?? $s?->line2_position_y ?? 1000); ?>,
            line3FontFamilyFemale: <?php echo json_encode($s?->line3_font_family_female ?? $s?->line3_font_family ?? 'Cairo', 15, 512) ?>,
            line3FontSizeFemale: <?php echo e($s?->line3_font_size_female ?? $s?->line3_font_size ?? 50); ?>,
            line3ColorFemale: <?php echo json_encode($s?->line3_color_female ?? $s?->line3_color ?? '#374151', 15, 512) ?>,
            line3PositionXFemale: <?php echo e($s?->line3_position_x_female ?? $s?->line3_position_x ?? 1240); ?>,
            line3PositionYFemale: <?php echo e($s?->line3_position_y_female ?? $s?->line3_position_y ?? 1100); ?>,
            line4FontFamilyFemale: <?php echo json_encode($s?->line4_font_family_female ?? $s?->line4_font_family ?? 'Cairo', 15, 512) ?>,
            line4FontSizeFemale: <?php echo e($s?->line4_font_size_female ?? $s?->line4_font_size ?? 50); ?>,
            line4ColorFemale: <?php echo json_encode($s?->line4_color_female ?? $s?->line4_color ?? '#374151', 15, 512) ?>,
            line4PositionXFemale: <?php echo e($s?->line4_position_x_female ?? $s?->line4_position_x ?? 1240); ?>,
            line4PositionYFemale: <?php echo e($s?->line4_position_y_female ?? $s?->line4_position_y ?? 1200); ?>,
            line5FontFamilyFemale: <?php echo json_encode($s?->line5_font_family_female ?? $s?->line5_font_family ?? 'Cairo', 15, 512) ?>,
            line5FontSizeFemale: <?php echo e($s?->line5_font_size_female ?? $s?->line5_font_size ?? 50); ?>,
            line5ColorFemale: <?php echo json_encode($s?->line5_color_female ?? $s?->line5_color ?? '#374151', 15, 512) ?>,
            line5PositionXFemale: <?php echo e($s?->line5_position_x_female ?? $s?->line5_position_x ?? 1240); ?>,
            line5PositionYFemale: <?php echo e($s?->line5_position_y_female ?? $s?->line5_position_y ?? 1300); ?>,
            line6FontFamilyFemale: <?php echo json_encode($s?->line6_font_family_female ?? $s?->line6_font_family ?? 'Cairo', 15, 512) ?>,
            line6FontSizeFemale: <?php echo e($s?->line6_font_size_female ?? $s?->line6_font_size ?? 50); ?>,
            line6ColorFemale: <?php echo json_encode($s?->line6_color_female ?? $s?->line6_color ?? '#374151', 15, 512) ?>,
            line6PositionXFemale: <?php echo e($s?->line6_position_x_female ?? $s?->line6_position_x ?? 1240); ?>,
            line6PositionYFemale: <?php echo e($s?->line6_position_y_female ?? $s?->line6_position_y ?? 1400); ?>,
            
            // التوقيعات
            signatureLeftText: <?php echo json_encode($s?->signature_left_text ?? 'مدير المدرسة', 15, 512) ?>,
            signatureLeftFontFamily: <?php echo json_encode($s?->signature_left_font_family ?? 'Cairo', 15, 512) ?>,
            signatureLeftFontSize: <?php echo e($s?->signature_left_font_size ?? 45); ?>,
            signatureLeftColor: <?php echo json_encode($s?->signature_left_color ?? '#1e3a8a', 15, 512) ?>,
            signatureLeftPositionX: <?php echo e($s?->signature_left_position_x ?? 620); ?>,
            signatureLeftPositionY: <?php echo e($s?->signature_left_position_y ?? 1500); ?>,
            
            signatureRightText: <?php echo json_encode($s?->signature_right_text ?? 'الكادر الإداري', 15, 512) ?>,
            signatureRightFontFamily: <?php echo json_encode($s?->signature_right_font_family ?? 'Cairo', 15, 512) ?>,
            signatureRightFontSize: <?php echo e($s?->signature_right_font_size ?? 45); ?>,
            signatureRightColor: <?php echo json_encode($s?->signature_right_color ?? '#1e3a8a', 15, 512) ?>,
            signatureRightPositionX: <?php echo e($s?->signature_right_position_x ?? 1860); ?>,
            signatureRightPositionY: <?php echo e($s?->signature_right_position_y ?? 1500); ?>,
            
            // إعدادات الاسم - ذكور
            namePositionX: <?php echo e($s?->name_position_x ?? 1240); ?>,
            namePositionY: <?php echo e($s?->name_position_y ?? 700); ?>,
            // إعدادات الاسم - إناث
            namePositionXFemale: <?php echo e($s?->name_position_x_female ?? $s?->name_position_x ?? 1240); ?>,
            namePositionYFemale: <?php echo e($s?->name_position_y_female ?? $s?->name_position_y ?? 700); ?>,
            nameFontFamily: <?php echo json_encode($s?->name_font_family ?? 'Cairo', 15, 512) ?>,
            nameFontSize: <?php echo e($s?->name_font_size ?? 80); ?>,
            primaryColor: <?php echo json_encode($s?->primary_color ?? '#1e3a8a', 15, 512) ?>,
            fontFamily: <?php echo json_encode($s?->font_family ?? $s?->name_font_family ?? 'Cairo', 15, 512) ?>,
            
            showDate: <?php echo e($s?->show_date ?? true ? 'true' : 'false'); ?>,
        },
        ...
        loadTemplate() {
            ...
            this.bgImage.src = '<?php echo e(url("public/storage/" . $s->background_image)); ?>';
            <?php else: ?>
            this.bgImage.src = '<?php echo e(url("public/images/certificate_template_v2.png")); ?>';
            <?php endif; ?>
...
<?php echo $__env->make('layouts.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/ntegty/public_html/resources/views/certificate/index.blade.php ENDPATH**/ ?>